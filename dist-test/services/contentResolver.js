...
class ContentResolver {
    constructor() {
        this.cache = new Map(); // lang -> stringId -> text
        this.initialized = false;
    }
    /**
     * Resolve an exercise's string IDs to localized text
     */
    async resolveExercise(exercise, targetLang) {
        ...
        const steps = exercise.stepsStringIds && exercise.stepsStringIds.length > 0
            ? exercise.stepsStringIds.map((stringId, idx) => langCache[stringId] || exercise.steps[idx] || '')
            : exercise.steps;
        // Resolve warning
     ...
     */
    async resolveExercises(exercises, targetLang) {
     ...
     */
    async getTranslation(stringId, targetLang) {
        const lang = targetLang || getUserLanguage();
        if (lang === 'fr') {
...
export const resolveExercise = (exercise, lang) => {
    return contentResolver.resolveExercise(exercise, lang);
};
export const resolveExercises = (exercises, lang) => {
